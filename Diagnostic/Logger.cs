...
namespace Diagnostic
{
    ...
    public class Logger : FileHandler
    {
        ...
        private static void InitializeFile(string path)
        {
            ...
            {
                string header = $"UTC time: {GetUtcDateTime()}, current time zone ({TimeZone.CurrentTimeZone.StandardName}) time: {GetDateTime()}{Environment.NewLine}" +
        ...
        private static string ResizeString(string str)
        {
            string resized = str.Length > MaxSourceLenght ?
                new string(str.Take(MaxSourceLenght).ToArray()) :
